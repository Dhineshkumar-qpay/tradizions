...
export default function ProfilePage() {
  ...
  const fetchWallet = async () => {
    ...
    try {
      const response = await API.get(API_ROUTES.GET_WALLET_BALANCE);
      if (response.status === 200) {
        setWalletPoints(response.data?.wallet?.points || 0);
      }
    } catch (err) {
          ...
          <div className="lg:col-span-2 relative p-8 rounded-xl bg-white border border-gray-200 shadow-sm flex flex-col justify-between">
            ...
            <div>
              ...
              <div className="flex gap-3">
                ...
                <button className="px-6 py-3 rounded bg-white hover:bg-gray-50 border border-gray-200 text-gray-700 text-[11px] font-bold tracking-widest uppercase transition-colors">
                  View Statements
                </button>
              </div>
            </div>
